...
statement=db.incdatas.find_one({'useradinp' : uid})
...
for strn in statement:
    ...
    if(len(strn)==0 or strn=='end'):
        ...
        dic.close()
        lj=str(lj)
        db.retdatas.update_one({'useradresp' : uid}, {'$set' : {'lj' : lj}})
        cj=str(cj)
        db.retdatas.update_one({'useradresp' : uid}, {'$set' : {'cj' : cj}})
        polyjobs=''
        for x in pj:
